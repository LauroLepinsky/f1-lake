...
WITH
  ...
  SELECT
    ...
    sum( CASE WHEN points > 0 THEN 1 ELSE 0 END ) AS qtd_sessions_points,
    sum( CASE WHEN mode = 'Race' AND points > 0 THEN 1 ELSE 0 END ) AS qtd_sessions_points_race,
    sum( CASE WHEN mode = 'Sprint' AND points > 0 THEN 1 ELSE 0 END ) AS qtd_sessions_points_sprint,
...
(
    nekt.load_table(
        layer_name="Bronze", table_name="f1_results"
...
for y in years:
    ...
    dates = (
        spark.sql(query_dates.format(year=y)).toPandas()["dt_ref"].astype(str).tolist()
    )
