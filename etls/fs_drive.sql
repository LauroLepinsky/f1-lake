WITH -- 1. Determina todas as datas de referência únicas (corridas de 2024 a 2025)
dim_dates AS (
	SELECT DISTINCT date(date) AS dt_ref,
		year AS ref_year
	FROM f1_results
	WHERE year >= 2024
		AND year <= 2025 -- isso é parametrizável
),
...
past_sessions AS (
	SELECT d.dt_ref,
		...
		f.*
	FROM dim_dates d
		INNER JOIN f1_results as f ON date(f.date) <= d.dt_ref
),
...
tb_results AS (
	SELECT p.*
	FROM past_sessions p
		INNER JOIN eligible_drivers e ON p.dt_ref = e.dt_ref
		AND p.driverid = e.driverid
		INNER JOIN last_rounds r ON p.dt_ref = r.dt_ref
		AND p.year = r.year
